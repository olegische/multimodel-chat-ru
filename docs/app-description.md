# Описание веб-приложения "Multimodel Chat"

---

## 1. Обзор

Приложение представляет собой веб-чат для взаимодействия с Yandex GPT, аналогичный интерфейсу ChatGPT.  
Основные характеристики:

- **Целевая аудитория**: пользователи, желающие общаться с Yandex GPT для генерации текстов, получения ответов или ведения бесед.
- **MVP-функционал**:
...
### **Frontend**:
- **Next.js** — фреймворк для создания веб-приложений на основе React, поддерживающий серверный рендеринг и статическую генерацию.
- **Tailwind CSS** — для стилизации интерфейса.

### **Backend**:
- **Node.js** — серверная платформа для выполнения JavaScript на сервере.
- **Prisma ORM** — упрощает взаимодействие с базой данных, предоставляет удобный API для работы с SQLite и поддерживает миграции.
- **SQLite** — легковесная реляционная база данных, встроенная непосредственно в приложение.
- **Yandex GPT API** — для обработки текстовых запросов.

### **База данных (в будущем релизе)**:
- **SQLite** — хранение данных, например, контекста беседы, в рамках простого проекта.

### **Развёртывание**:
- **Yandex Cloud** — хостинг бэкенда и доступ к GPT.
...
**Цель**: Предоставить интерфейс для общения с Yandex GPT.  
**Элементы**:
- Поле ввода текста с кнопкой отправки.
- Лента сообщений (вопросы пользователя и ответы GPT).
- Настройки генерации текста (температура, длина ответа).
...
- **Функциональность**:
  ...
  - **Переключатель темы**: Позволяет пользователю переключать между светлой и тёмной темой. Это состояние нужно сохранять в **localStorage** для сохранения настроек при перезагрузке страницы.

- **Взаимодействие с API**:
  - Не требуется взаимодействие с сервером, так как переключение темы — это локальное состояние.
...
- При открытии приложения, пользователю отображается главная страница с интерфейсом чата, включая поле ввода для текста и область для истории сообщений. В интерфейсе также отображается кнопка для отправки сообщения и параметры для настройки генерации текста (например, температура, длина ответа).
